...
# fohlsso2 2023-08-06
# modified to work with linux, and maybe with ssd:s too, at least the two brands I have
# the first line still needs to be manually edited for linux/bsd path (yes I'm lazy :)  )
# linux is /usr/bin/bash and bsd is /usr/local/bin/bash
# Reason for my version is to make it work with a third fan zone and eight fans
# because my test board Supermicro X10SRG-F splits the peripheral fans into two zones
# in the config file you can disable or enable the third zone using ZONE_THIRD= -1 / 0 / 1

# The way the third zone is implemented in the script is that it still only calculates values for two groups
# and then applies the logic to the different zones.
# Because we are doing NAS here, and we only care about drives and cpu's anyway.

# If you want to edit this script to suit your needs you can safely leave everything related to the third zone
# in the script, as long as you don't enable it in the config it will not act on anything.
# The values it calculates might be very wrong, but that doesn't matter as they are not used.
...
##############################################
# enable or disable third zone operations
##############################################

if [[ $ZONE_THIRD -gt 0 ]]; then
    echo "Third Zone logic active."
    THIRDZONE=true
else
    THIRDZONE=false
    echo "Going with two zones only today."
fi
...
function print_header {
   ...
   printf "\n%-*s %3s %16s %29s \n" $SPACES "$DATE" "CPU" "New_Fan%" "New_RPM_____________________"  # Fredo - fixa spaces här!
   echo -n "          "
...
#   printf "%4s %5s %5s %3s %5s %5s %5s %5s %5s %5s %5s %-7s" "Tmax" "Tmean" "ERRc" "CPU" "FAN1" "FAN2" "FAN3" "FAN4" "FANA" "Fan%0" "Fan%1" "MODE" 
   ...
   printf "%4s %5s %5s %5s %5s %5s %5s %5s %5s %5s %5s %5s %5s %5s %5s %-7s" "Tmax" "Tmean" "ERRc" "CPU" "FAN1" "FAN2" "FAN3" "FAN4" "FANA" "FANB" "FANC" "FAND" "Fan%0" "Fan%1" "Fan%2" "MODE" 
}
...
function read_fan_data {
   ...
   # the script will assume the duty cycles are what was last set.
	if [[ $HOW_DUTY == 1 ]] ; then
		...
		DUTY_HDD1=$($IPMITOOL raw 0x30 0x70 0x66 0 $ZONE_HDD)
		DUTY_HDD1=$((0x$(echo $DUTY_HDD1)))
        if [[ "$THIRDZONE" = true ]]; then
            DUTY_HDD2=$($IPMITOOL raw 0x30 0x70 0x66 0 $ZONE_HDD2)
            DUTY_HDD2=$((0x$(echo $DUTY_HDD2)))
        fi
	fi
   ...
   # Comment out or remove the fans you don't need, remeber to match it with the list of columns above
   SDR=$($IPMITOOL sdr)
...
function CPU_check_adjust {
   ...
   # Use this short CPU cycle to also allow HDD fans to come down 
   ...
   # drive fans to DUTY_HDD_MIN, but that's ok if drives are that cool.
   # However, this is experimental.
	if [[ PD -lt 0 && (( $(bc <<< "scale=2; $Tmean < ($SP-1)") == 1 )) ]]; then
		DUTY_HDD_LAST=$DUTY_HDD
		DUTY_HDD2_LAST=$DUTY_HDD2
		DUTY_HDD=$(( DUTY_HDD1 + PD ))
		DUTY_HDD2=$(( DUTY_HDD2 + PD ))
		# Don't allow duty cycle below min
		if [[ $DUTY_HDD -lt $DUTY_HDD_MIN ]]; then DUTY_HDD=$DUTY_HDD_MIN; fi
		if [[ $DUTY_HDD2 -lt $DUTY_HDD_MIN ]]; then DUTY_HDD2=$DUTY_HDD_MIN; fi
		# pass to the function adjust_fans
		adjust_fans $ZONE_HDD $DUTY_HDD $DUTY_HDD1_LAST

        if [[ "$THIRDZONE" = true ]]; then
		    adjust_fans $ZONE_HDD2 $DUTY_HDD2 $DUTY_HDD2_LAST
        fi
	fi
...
function DRIVES_check_adjust {
   ...
   i=0             # initialize count of active drives
   while read -r LINE ; do
      get_disk_name
      ...
      if [ "$STATUS" == "*" ] ; then
         ...
         # hopefully we can find our ssd:s here, this matches my old Intel DC3510 at least
         # if it doesnt match yours, just modify or keep adding more ELIF lines as needed....
         elif grep -Fq "Temperature_Internal" /var/tempfile ; then
         	TEMP=$( cat /var/tempfile | grep "Temperature_Internal" | awk '{print $10}')

        # And one SSD report temp using 'Airflow_Temperature_Cel' so I add that one too (Samsung)
         elif grep -Fq "Airflow_Temperature_Cel" /var/tempfile ; then
         	TEMP=$( cat /var/tempfile | grep "Airflow_Temperature_Cel" | awk '{print $10}')
         else
   ...
   # if no disks are active
	if [ $i -eq 0 ]; then
		Tmean=""; Tmax=""; P=""; D=""; ERRc=""
		DUTY_HDD=$DUTY_HDD_MIN
		DUTY_HDD2=$DUTY_HDD_MIN
	else
	# summarize, calculate PD and print Tmax and Tmean
		...
		let "DUTY_HDD1 = $DUTY_HDD_LAST + $PD"
		let "DUTY_HDD2 = $DUTY_HDD2_LAST + $PD"

		# Don't allow duty cycle outside min-max
		if [[ $DUTY_HDD -gt $DUTY_HDD_MAX ]]; then DUTY_HDD=$DUTY_HDD_MAX; fi
		if [[ $DUTY_HDD -lt $DUTY_HDD_MIN ]]; then DUTY_HDD=$DUTY_HDD_MIN; fi
		if [[ $DUTY_HDD2 -gt $DUTY_HDD_MAX ]]; then DUTY_HDD2=$DUTY_HDD_MAX; fi
		if [[ $DUTY_HDD2 -lt $DUTY_HDD_MIN ]]; then DUTY_HDD2=$DUTY_HDD_MIN; fi
	fi

   # DIAGNOSTIC variables - uncomment for troubleshooting:
   # printf "\n DUTY_HDD=%s, DUTY_HDD_LAST=%s, DUTY=%s, Tmean=%s, ERRp=%s \n" "${DUTY_HDD:---}" "${DUTY_HDD_LAST:---}" "${DUTY:---}" "${Tmean:---}" $ERRp

   # pass to the function adjust_fans
   adjust_fans $ZONE_HDD $DUTY_HDD $DUTY_HDD_LAST
   if [[ "$THIRDZONE" = true ]]; then adjust_fans $ZONE_HDD2 $DUTY_HDD2 $DUTY_HDD2_LAST; fi
...
function mismatch_test {
	MISMATCH=0; MISMATCH_CPU=0; MISMATCH_HDD=0
	...
	if [[ (DUTY_HDD -ge 95 && ${!RPM_HDD} -lt RPM_HDD_MAX) || (DUTY_HDD -lt 25 && ${!RPM_HDD} -gt RPM_HDD_30) ]] ; then
		MISMATCH=1; MISMATCH_HDD=1
		printf "\n%s\n" "Mismatch between HDD Duty and RPMs -- DUTY_HDD=$DUTY_HDD; RPM_HDD=${!RPM_HDD}"
	fi
    if [[ "$THIRDZONE" = true ]]; then
        if [[ (DUTY_HDD2 -ge 95 && ${!RPM_HDD} -lt RPM_HDD_MAX) || (DUTY_HDD2 -lt 25 && ${!RPM_HDD} -gt RPM_HDD_30) ]] ; then
            MISMATCH=1; MISMATCH_HDD2=1
            printf "\n%s\n" "Mismatch between HDD Duty and RPMs -- DUTY_HDD=$DUTY_HDD2; RPM_HDD=${!RPM_HDD}"
        fi
    fi
...
function force_set_fans {
	...
	if [ $MISMATCH_HDD == 1 ]; then
		FIRST_TIME=1
		adjust_fans $ZONE_HDD $DUTY_HDD $DUTY_HDD_LAST
		echo "Attempting to fix HDD mismatch  "
		sleep 5
	fi
}
...
printf "\n****** SETTINGS ******\n"
printf "CPU zone %s; Peripheral/HDD zone %s\n" $ZONE_CPU $ZONE_HDD
printf "CPU fans min/max duty cycle: %s/%s\n" $DUTY_CPU_MIN $DUTY_CPU_MAX
printf "HDD fans min/max duty cycle: %s/%s\n" $DUTY_HDD_MIN $DUTY_HDD_MAX
printf "CPU fans - measured RPMs at 30%% and 100%% duty cycle: %s/%s\n" $RPM_CPU_30 $RPM_CPU_MAX
printf "HDD fans - measured RPMs at 30%% and 100%% duty cycle: %s/%s\n" $RPM_HDD_30 $RPM_HDD_MAX
...
printf "CPU scalar: %s\n" $CPU_SCALE
if [[ "$THIRDZONE" = true ]]; then printf "Third Zone: ACTIVE" ; fi
...
RPM_HDD_30=$(echo "scale=0; 1.2 * $RPM_HDD_30 / 1" | bc)
RPM_HDD_MAX=$(echo "scale=0; 0.8 * $RPM_HDD_MAX / 1" | bc)
...
### Zone 1 - Peripheral/HDD fans, headers with letter (e.g., FANA, FANB, etc.)

if [[ ZONE_HDD -eq 0 ]]; then
   RPM_HDD=FAN1
   RPM_CPU=FANA
else
   RPM_HDD=FANA
...
if [[ "$THIRDZONE" = true ]]; then RPM_HDD2=FANC ; fi
ZONE_HDD2=2
...
if [[ ${!RPM_HDD} -ge RPM_HDD_MAX || -z ${DUTY_HDD+x} ]]; then
   echo -n "$($IPMITOOL raw 0x30 0x70 0x66 1 $ZONE_HDD 50)"
   DUTY_HDD=50; sleep 1
fi
...
if [[ "$THIRDZONE" = true ]]; then
    if [[ ${!RPM_HDD2} -ge RPM_HDD_MAX || -z ${DUTY_HDD2+x} ]]; then
    echo -n "$($IPMITOOL raw 0x30 0x70 0x66 1 2 50)"
    DUTY_HDD2=50; sleep 1
    fi
fi
...
# not sure i got the spaces right here :) Fredo fixa, fel ordning vs headern ffs
   printf "%7s %6s %6.6s %4s %-7s %3d %3d %6s %5s %5s %5s %5s %5s %5s %5s %5s" "${ERRc:----}" "${P:----}" "${D:----}" "$CPU_TEMP" $MODEt $DUTY_CPU $DUTY_HDD $DUTY_HDD2 "${FANA:----}" "${FANB:----}" "${FANC:----}" "${FAND:----}" "${FAN1:----}" "${FAN2:----}" "${FAN3:----}" "${FAN4:----}"
